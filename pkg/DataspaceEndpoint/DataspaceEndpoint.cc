...
void * DataspaceEndpoint::server_intf_getter(void * args)
{
...
int DataspaceEndpoint::start_geting_server_intf()
{
  int rc = pthread_create(&th_peer_intf, &attr, server_intf_getter, (void*)this);
  return rc;
}

void * DataspaceEndpoint::localMemoryManager_initializer_th( void * arg)
{
...
int DataspaceEndpoint::start_init_LocalMemoryManager()
{
  int rc = pthread_create(&th_local_mem, &attr, localMemoryManager_initializer_th, (void*)this);
  return rc;
}

void * DataspaceEndpoint::free_peer_req_callback_thread(void * arg)
{
...
int DataspaceEndpoint::free_peer_req_callback(u_int64_t id ,u_int8_t type, l4_size_t write_index, l4_size_t size)
{
...
int DataspaceEndpoint::new_data_callback_priv(u_int64_t id ,u_int8_t type, l4_size_t write_index, l4_size_t size)
{
...
int DataspaceEndpoint::new_data_callback_default(u_int64_t id, u_int8_t type, u_int8_t * read_addr , l4_size_t size)
{
  // run in a thread !
...
DataspaceEndpoint::DataspaceEndpoint(
  L4Re::Util::Registry_server<> *server,
  const char *CTS_ipc_name,
  const char *STC_ipc_name,
  l4_size_t   peer_size,
  u_int64_t    peer_timeout,
  std::function<int(DataspaceEndpoint* obj,u_int64_t id ,u_int8_t type, const u_int8_t * addr, l4_size_t size)> new_req_callback_handler_
):
...
void * DataspaceEndpoint::allocate_local( l4_size_t size)
{
...
void * DataspaceEndpoint::allocate_local_blocking(l4_size_t size)
{
...
void * DataspaceEndpoint::add_new_req_worker(void * arg)
{
  WorkerArgs * p = (WorkerArgs*) arg;
...
int DataspaceEndpoint::add_new_req(u_int64_t id ,u_int8_t type, void * addr, l4_size_t size)
{
...
void * DataspaceEndpoint::free_peer_req_worker(void* arg)
{
  WorkerArgs * p = (WorkerArgs*) arg;
...
int DataspaceEndpoint::free_peer_req(u_int64_t id ,u_int8_t type, const void * addr, l4_size_t size)
{
...
bool DataspaceEndpoint::wait_for_initialization(int timeout)
{
