#ifndef __CRYPTO_SHARED2__
#define __CRYPTO_SHARED2__
...
struct IDataspaceOwner2 : L4::Kobject_t<IDataspaceOwner2, L4::Kobject, 0x48>
{
...
class DataspaceOwner2 : public L4::Epiface_t<DataspaceOwner2, IDataspaceOwner2>
{
  ...
  ) 
  {
    ...
    if (!ds.is_valid()) {
      std::printf("DataspaceOwner2: Capability allocation failed\n");
      return;
    }
  ...
  int op_init(IDataspaceOwner2::Rights, const l4_size_t size, const u_int64_t timeout)
  {
    std::printf("DataspaceOwner2: init called size=%lu, timeout=%lu\n",
    ...
    if (err < 0) {
      std::printf("DataspaceOwner2: Memory allocation failed: %ld\n", err);
      return 1;
    }
    std::printf("DataspaceOwner2: Dataspace created successfully, size=%lu bytes\n",
    ...
    if (err < 0) {
      std::printf("DataspaceOwner2: attach_ds: attach failed (%ld)\n", err);
      return 1;
    }
    else {
      std::printf("DataspaceOwner2: attach_ds: attached at %p, size=%lu\n",
                  addr, static_cast<unsigned long>(size_ds));
      p_ds = addr;
    } 
    return 0; // not implemented
  }
  int op_getDS(IDataspaceOwner2::Rights, L4::Ipc::Cap<L4Re::Dataspace> &out_ds)
  {
    out_ds = L4::Ipc::make_cap(ds, L4_CAP_FPAGE_RW);
    return L4_EOK;
  }
  int op_new_data(IDataspaceOwner2::Rights, u_int64_t id ,u_int8_t type, l4_size_t write_index, l4_size_t size)
  {
    std::printf("DataspaceOwner2: new_data called id=%lu, type=%u, write_index=%lu, size=%lu\n",
                static_cast<unsigned long>(id),
                static_cast<unsigned int>(type),
                static_cast<unsigned long>(write_index),
                static_cast<unsigned long>(size));    
    if (p_ds != nullptr) {
      std::printf("DataspaceOwner2 read from dataspace: %s\n", (static_cast<char*>(p_ds) + write_index));
      return L4_EOK;
    }
    else {
      std::printf("DataspaceOwner2: dataspace pointer is null\n");
      return 1;
    }
    if (new_data_callback_fn) {
      ...
      std::printf("DataspaceOwner2: new_data_callback_fn is null\n");
    
    return L4_EOK;
  }

  int op_free_your_data(IDataspaceOwner2::Rights, u_int64_t id ,u_int8_t type, l4_size_t write_index, l4_size_t size)
  {
    std::printf("DataspaceOwner2: free_your_data called id=%lu, type=%u, write_index=%lu, size=%lu\n",
                static_cast<unsigned long>(id),
                static_cast<unsigned int>(type),
                static_cast<unsigned long>(write_index),
                static_cast<unsigned long>(size));    
    if (free_your_data_callback_fn) {
      ...
      std::printf("DataspaceOwner2: free_your_data_callback_fn is null\n");
    return L4_EOK;
  } 

  int op_placeholder(IDataspaceOwner2::Rights)
  {
    std::printf("DataspaceOwner2: placeholder called\n");
    return 0;
  }
};
/*
...
{
private:
  std::unique_ptr<DataspaceOwner2> this_side;  // null initially
...
public:
  L4::Cap<IDataspaceOwner2> other_side ;
  DS_Side(
  ...
  ) 
  {
    ...
    {
        this_side = std::make_unique<DataspaceOwner2>(
        server, 
        this_side_ipc_name,
        new_data_callback,
    ...
    {
      other_side =  L4::Cap<IDataspaceOwner2>();
      other_side =L4Re::Env::env()->get_cap<IDataspaceOwner2>(other_side_ipc_name);
